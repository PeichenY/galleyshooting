class Second extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image("hippo2", "hippo.png");
    ...
    create() {
        ...
        my.sprite.hippoHorizontal = this.add.sprite(game.config.width/2, 80, "hippo2");
        my.sprite.hippoHorizontal.setScale(0.25);
        my.sprite.hippoHorizontal.setDepth(1);
        my.sprite.hippoHorizontal.scorePoints = 25;
        my.sprite.hippoHorizontal.velocity = Phaser.Math.Between(-10,10);
        ...
        this.time.addEvent({
            delay: 1000,
            callback: this.fireHippoHorizontalBullet,
            callbackScope: this,
            loop: true
        });
    ...
    update() {
        ...
        //hippo horizontal
        my.sprite.hippoHorizontal.x += my.sprite.hippoHorizontal.velocity;
        if (my.sprite.hippoHorizontal.x <= 0 || my.sprite.hippoHorizontal.x >= game.config.width) {
            my.sprite.hippoHorizontal.velocity *= -1; // 达到水平边界时反弹
             }

        // Move the hippo
        my.sprite.hippo.y += my.sprite.hippo.velocity;
        if (my.sprite.hippo.y <= 0) {  // Assuming the top of the screen is y = 0
            my.sprite.hippo.velocity *= -1;}
            else if (my.sprite.hippo.y >= my.sprite.elephant.y - my.sprite.elephant.displayHeight / 2) {
                my.sprite.hippo.velocity *= -1;
            }
        ...
        my.sprite.hippoBulletsHorizontal = my.sprite.hippoBulletsHorizontal.filter(bullet => {
            bullet.y += 3;  // 子弹的移动速度
        ...
        if (this.collides(my.sprite.hippo, my.sprite.elephant)) {
            my.sprite.elephant.hp = 0;
            this.resetGameState();
        ...
        for (let bullet of my.sprite.bullet) {
            if (this.collides(my.sprite.hippo, bullet, my.sprite.hippoHorizontal)) {
                // start animation
                ...
                my.sprite.hippoHorizontal.visible = false;
                my.sprite.hippoHorizontal.x = -100;

                // Update score
                this.myScore += my.sprite.hippo.scorePoints;
                this.myScore += my.sprite.hippoHorizontal.scorePoints;
                ...
                this.puff.on(Phaser.Animations.Events.ANIMATION_COMPLETE, () => {
                    ...
                    this.my.sprite.hippoHorizontal.visible = true;

                    this.my.sprite.hippo.x = Math.random()*config.width;
                    this.my.sprite.hippoHorizontal.x = Math.random()*config.width;
                }, this);
    ...
    fireHippoHorizontalBullet() {
        if (this.my.sprite.hippoBulletsHorizontal.length < this.maxHippoBulletsHorizontal) {
            let Hbullet = this.add.sprite(this.my.sprite.hippoHorizontal.x, this.my.sprite.hippoHorizontal.y, 'HB');
            Hbullet.setScale(0.1);
            this.my.sprite.hippoBulletsHorizontal.push(Hbullet);
        }
    }
